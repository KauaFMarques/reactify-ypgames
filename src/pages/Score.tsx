...
const Score = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await fetchScore(room, stage);
        setTeams(data);
      } catch {
        setTeams([]);
      }
  ...
  const sortedTeams = teams
    ? [...teams].sort((a, b) => b.score - a.score || a.avg_time_ms - b.avg_time_ms)
    : [];
      ...
      <header className="text-center mb-8">
        ...
        <div className="flex justify-center gap-2 mt-4 flex-wrap">
          {[undefined, 1, 2, 3, 4].map((s) => (
            ...
            >
              {s === undefined ? "Todas as etapas" : `Etapa ${s}`}
            </button>
          ))}
        </div>
      </header>
        ...
        {loading ? (
          ...
          <div className="bg-card rounded-2xl shadow-xl overflow-hidden">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {["#", "Equipe", "Melhores Decisões", "Tempo Médio", "Detalhes"].map((h) => (
                    <th
                      key={h}
              ...
              <tbody>
                {sortedTeams.map((team, i) => (
                  ...
                  >
                    ...
                    <td className="p-4 font-semibold text-foreground">{team.team_name}</td>
                    <td className="p-4">
                      <span className="font-bold text-primary text-xl">{team.score}</span>
                      <span className="text-muted-foreground text-sm"> / {team.total}</span>
                    </td>
